...
import rag_api
from chatbot import _get_vertex_location, _resolve_provider
...
def generate_feedback(
    ...
    if dry_run or not api_key:
        reason = "DRY-RUN" if dry_run else "LLM provider is not configured. Set GCP_PROJECT_ID and Google credentials for Vertex AI."
        feedback = {
            "summary": reason,
            "key_points": [],
            "warnings": [],
...
def generate_image_feedback(
    ...
    """업로드된 작업 사진 + RAG 노하우를 함께 보고 구조화된 피드백 반환."""
    knowhow = rag_api.get_knowhow(
        material=material,
        position=position,
        query=observation,
    ...
    if dry_run or not api_key:
        ...
        feedback = _call_vertex_image_json(
            system=SYSTEM_PROMPT,
            knowhow_str=_format_knowhow_for_prompt(knowhow),
            observation=observation or "",
        ...
        feedback = _call_gemini_image_json(
            system=SYSTEM_PROMPT,
            knowhow_str=_format_knowhow_for_prompt(knowhow),
            observation=observation or "",
            image_bytes=image_bytes,
        ...
        feedback = _call_groq_json(
            system=SYSTEM_PROMPT,
            knowhow_str=_format_knowhow_for_prompt(knowhow),
            observation=(observation or "") + "\n\n[작업 사진은 업로드됐지만 현재 LLM 공급자는 이미지 입력을 지원하지 않아 텍스트/RAG 기준으로만 피드백합니다.]",
            api_key=api_key,
            model=model,
        )
        llm_info = {
...
def _build_image_user_msg(knowhow_str: str, observation: str) -> str:
    return (
        f"<knowhow>\n{knowhow_str}\n</knowhow>\n\n"
        f"<observation>\n{observation or '(사용자가 상황 설명을 제공하지 않음. 업로드 사진과 knowhow 기준으로.)'}\n</observation>\n\n"
        "첨부된 작업 사진을 먼저 관찰하고, 보이는 비드/색상/오염/언더컷/용융 상태를 "
        "knowhow와 대조해 피드백 JSON을 생성하라. 사진만으로 확정할 수 없는 내용은 "
        "단정하지 말고 확인 항목으로 표현하라. 반드시 JSON 객체만 반환 (코드 펜스 없이)."
    )
...
def _call_vertex_json(
    ...
    try:
        ...
        return {
            "summary": f"[google-genai SDK missing: pip install google-genai] {e}",
            "key_points": [], "warnings": [], "next_steps": [],
        }
    ...
    try:
        ...
        return {
            "summary": f"[Vertex AI call failed project={project} location={location} model={model}] {type(e).__name__}: {e}",
            "key_points": [], "warnings": [], "next_steps": [],
        }
    ...
    try:
        ...
        return {
            "summary": "[Vertex AI JSON parse failed]",
            "key_points": [raw[:500]],
            "warnings": [], "next_steps": [],
        }


def _call_gemini_image_json(
    system: str,
    knowhow_str: str,
    observation: str,
    image_bytes: bytes,
    ...
    try:
        resp = client.models.generate_content(
            ...
            contents=[
                types.Content(
                    role="user",
                    parts=[
                        types.Part.from_text(text=_build_image_user_msg(knowhow_str, observation)),
        ...
        return {
            "summary": f"[Gemini image call failed] {type(e).__name__}: {e}",
            "key_points": [], "warnings": [], "next_steps": [],
        }
    ...
    try:
        ...
        return {
            "summary": "[Gemini image JSON parse failed]",
            "key_points": [raw[:500]],
            "warnings": [], "next_steps": [],
        }


def _call_vertex_image_json(
    system: str,
    knowhow_str: str,
    observation: str,
    image_bytes: bytes,
    ...
    try:
        ...
        return {
            "summary": f"[Vertex AI image call failed project={project} location={location} model={model}] {type(e).__name__}: {e}",
            "key_points": [], "warnings": [], "next_steps": [],
        }
    ...
    try:
        ...
        return {
            "summary": "[Vertex AI image JSON parse failed]",
            "key_points": [raw[:500]],
            "warnings": [], "next_steps": [],
        }
