...
FastAPI 서버 (rag_pipeline/server.py, :8000)
    ├─ POST /api/upload        ─ 사진/영상 업로드 + 메타 분류
    ...
    └─ GET  /docs              ─ Swagger UI (자동 생성)
        ...
        ├── agent.generate_feedback() / generate_image_feedback()
        │     └── knowhow + 선택 이미지 → Vertex/Gemini/Groq LLM 피드백
...
Git 업로드본에서는 대용량 mp4를 제외했다. 영상은 `전달_zip_민성` 또는 별도 공유 링크에서 받아 `dataset/video/`에 넣으면 재생된다.
추가 영상도 같은 폴더에 넣거나 `POST /api/videos/upload`로 업로드하면 된다.
...
| `POST` | `/api/feedback` | `{material,position,observation?,upload_id?}` | **Agent 피드백**. `upload_id`가 이미지면 이미지+RAG LLM 분석 |
...
const { upload_id, classification } = await (await fetch(`${API}/api/upload`, {
  method: "POST", body: fd
})).json()
...
const fb = await (await fetch(`${API}/api/feedback`, {
  ...
  body: JSON.stringify({
    ...
    upload_id,                             // 있으면 이미지+RAG 멀티모달 LLM 호출
  })
})).json()
